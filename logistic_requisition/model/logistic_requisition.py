...
class LogisticRequisition(models.Model):
    ...
    @api.multi
    #XXX dead code ? should it return ids ?
    def _store_get_requisition_line_ids(self):
        req_line_obj = self.env['logistic.requisition.line']
        line_ids = req_line_obj.search(
            [('requisition_id', 'in', self.env.ids)])
        return line_ids

    @api.multi
    @api.returns('logistic.requisition.line')
    def _get_all_lines(self):
        """ return aggregated lines of multiple requisitions """
        return [line.id for req in self for line in req.line_ids]
...
class LogisticRequisitionLine(models.Model):
    ...
    def onchange_product_id(self):
        ...
        """
        if self.product_id:
            self.requested_uom_id = self.product.uom_id
            self.description = self.product.name_get()[0][1]
        else:
...
class LogisticRequisitionSource(models.Model):
    ...
    offer_ids = fields.One2many(
        related='sale.order.line.logistic_requisition_source_id',
    ...
    supplier_partner_id = fields.Many2one(
        related='selected_bid_id.partner_id',
        relation='res.partner',
    ...
    _constraints = [
        (lambda self: self._check_purchase_requisition_unique(),
    ...
    @api.model
    def _is_sourced(self, source_id):
    ...
    @api.model
    def _prepare_po_requisition(self, sources, purch_req_lines,
                                pricelist=None):
        company_id = None
        user_id = None
        consignee_id = None
    ...
    @api.multi
    def action_create_po_requisition(self, pricelist=None):
        self._action_create_po_requisition(pricelist=pricelist)
    ...
    @api.multi
    def _get_total_cost(self, prop, unknow_none, unknow_dict):
        for line in self:
            line.amount_total = line.unit_cost * line.proposed_qty
    ...
    def onchange_selected_bid_id(self, selected_bid_id):
        ...
        if selected_bid_id:
            ...
            purchase = purchase_obj.browse(selected_bid_id)
